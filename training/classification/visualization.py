...
def plot_roc_curves(y_true, y_pred_probs, class_names, save_path=None):
    ...
    for class_idx in range(n_classes):
        ...
        for f, t in zip(fpr, tpr):
            curve_rows.append({"class": class_names[class_idx], "fpr": f, "tpr": t, "auc": roc_auc})
    ...
    if save_path:
        ...
        print(f"✅ Saved ROC curve data: {csv_path}")
...
def plot_precision_recall_curves(y_true, y_pred_probs, class_names, save_path=None):
    ...
    for class_idx in range(n_classes):
        ...
        for r, p in zip(recall, precision):
            curve_rows.append({"class": class_names[class_idx], "recall": r, "precision": p, "ap": ap})
    ...
    if save_path:
        ...
        import pandas as pd
        pd.DataFrame(curve_rows).to_csv(csv_path, index=False)
        print(f"✅ Saved PR curve data: {csv_path}")
...
def plot_calibration_curves(y_true, y_pred_probs, class_names, save_path=None, n_bins=10):
    ...
    for class_idx in range(n_classes):
        ...
        try:
            ...
            for pp, pt in zip(prob_pred, prob_true):
                curve_rows.append({"class": class_names[class_idx], "prob_pred": pp, "prob_true": pt, "ece": ece})
    ...
    if save_path:
        ...
        if curve_rows:
            ...
            import pandas as pd
            pd.DataFrame(curve_rows).to_csv(csv_path, index=False)
            print(f"✅ Saved calibration curve data: {csv_path}")
