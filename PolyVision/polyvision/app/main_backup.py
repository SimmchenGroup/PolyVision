...
class ImagePreview(QMainWindow):
    ...
    def on_delete_click_mode_toggled(self, state):
        ...
        if enabled:
            self.right_preview_view.setCursor(Qt.CrossCursor)
        else:
    ...
    def update_display(self):
        ...
        # ------------------- NEW: FUSION INFERENCE -------------------
        if self.use_fusion and self.use_yolo and len(self.bbox_list) > 0:
            # Get global context once per image
            global_output = self.global_clf.predict(self.current_img_gray, return_features=True)
            sample_context = self.get_context_type()  # from your context dropdown

            # For each bbox, fuse predictions
            for i, bbox in enumerate(self.bbox_list):
                # Skip if this bbox has a manual class override
                ...
                # Get YOLO output (convert class ID to probability distribution)
                yolo_probs = self._make_one_hot(self.last_yolo_pred_cls[i], 9)
                yolo_output = {
                    'probs': yolo_probs,
                ...
                # Get local classifier output
                try:
                    local_output = self.local_clf.predict(crop, return_features=True)
                except Exception as e:
                    print(f"Local classifier failed for bbox {i}: {e}")
                    continue
...
if __name__ == "__main__":
    ...
    # # === MODELS ===
    # # ---------------------------------------------------------------------
    YOLO_MODEL_PATH = r"C:/Users/joshk/OneDrive/Documents/GitHub_Strath/PolyVision/models/detect/YOLOv8/best.pt"
    ...
    # Load configuration
    with open("C:/Users/joshk/OneDrive/Documents/GitHub_Strath/PolyVision/configs/config.json", "r") as f:
        config = json.load(f)
    ...
    input_dir = Path(
        r"C:\Users\joshk\OneDrive\Desktop\raw\pet")
    exts = {".tif", ".tiff", ".png", ".jpg", ".jpeg"}
    ...
    output_root = Path(r"C:\Users\joshk\OneDrive\Desktop\raw\pet")
    raw_out = Path(r"C:\Users\joshk\OneDrive\Desktop\raw\pet\whole_images")
    ...
    previewer = ImagePreview(
        ...
        enable_fusion=config["models"]["enable_fusion"],
        local_model_path=config["models"]["local_classifier"],
        global_model_path=config["models"]["global_classifier"],
